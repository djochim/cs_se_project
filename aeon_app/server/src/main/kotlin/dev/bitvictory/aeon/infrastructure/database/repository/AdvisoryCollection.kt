...
import dev.bitvictory.aeon.core.domain.entities.advisory.ThreadId
...
class AdvisoryCollection(database: Database) : AdvisoryPersistence {
    ...
    override suspend fun getThreadId(id: ObjectId): String {
        logger.debug("Get Thread id of {}", id)
        val filter = Filters.eq("_id", id)
        val projection = Projections.fields(
            Projections.include(Advisory::threadId.name),
            Projections.excludeId()
        )
        return collection.find<ThreadId>(filter).projection(projection).first().threadId
    }
